...
- Instead of checking from each cell to the ocean, we reverse the flow and start DFS from ocean borders. We only move to neighboring cells with equal or higher
height, ensuring that water can flow back to the ocean. Finally, we take the intersection of cells reachable from both oceans
...
- Space
    O(M*N) no new data structure used all changes made in place
'''
...
class Solution:
    def pacificAtlantic(self, heights: List[List[int]]) -> List[List[int]]:
        ...
        for c in range(cols):
            dfs(0, c, pacific, -inf)
            dfs(rows - 1 , c, atlantic, -inf)
        
        # cols
        for r in range(rows):
            dfs(r, 0, pacific, -inf)
            dfs(r, cols - 1, atlantic, -inf)
